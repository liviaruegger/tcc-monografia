...
\begin{enumerate}
    ...
    \item \textbf{Tratamento de valores NoData}: Valores que representam ausência de dados (padding, NoData) devem ser identificados e tratados adequadamente, não sendo incluídos no cálculo da faixa de elevação.
    \item \textbf{Conversão de sistemas de coordenadas}: Dados em sistemas geográficos (graus) devem ter suas dimensões convertidas para metros, considerando a localização geográfica para cálculos precisos.
...
\begin{enumerate}
    ...
    \item[RF06] O sistema deve detectar e excluir automaticamente valores de padding/NoData nas bordas da imagem do cálculo da faixa de elevação, garantindo normalização correta dos dados.
...
\begin{figure}[p]
    ...
    \caption{Etapa 4: Logs de conversão. O plugin processa automaticamente (recorte, normalização 16-bit, detecção de padding, cálculo de dimensões), gera arquivo .raw compatível e apresenta logs.}
    \label{fig:prova-4}
\end{figure}
...
A solução implementada inclui a detecção do sistema de coordenadas do dado de entrada e o cálculo adequado das dimensões do terreno. Para sistemas geográficos, o plugin calcula as dimensões reais em metros considerando a localização geográfica dos dados, utilizando fórmulas de conversão que levam em conta a latitude média da área. Para sistemas projetados (como UTM), o cálculo é direto, pois as unidades já são métricas. O plugin também emite avisos quando detecta que o dado não está em um sistema projetado recomendado (como UTM), orientando o usuário sobre a importância da reprojeção prévia para resultados mais precisos.

\subsubsection{Problema do padding e lógica de contorno}

Durante os testes com dados reais, foi identificado que arquivos GeoTIFF frequentemente contêm valores de padding (geralmente zeros) nas bordas da imagem, especialmente após processos de reprojeção ou recorte. Esses valores de padding, quando incluídos no cálculo da faixa de elevação (mínimo e máximo), distorcem significativamente a normalização dos dados, resultando em terrenos com elevações incorretas no Unity.

Por exemplo, se um MDE tem valores de elevação reais entre 800 e 1200 metros, mas contém padding com valor zero nas bordas, o cálculo direto resultaria em uma faixa de 0 a 1200 metros. Isso faria com que a normalização para 16-bit escalasse incorretamente os valores, comprimindo a faixa útil de elevação e gerando um terreno visualmente distorcido.

A solução implementada inclui a detecção automática de padding através da análise das bordas da imagem raster. O algoritmo identifica valores que se repetem nas bordas (tipicamente zeros ou valores NoData) e os exclui do cálculo da faixa de elevação. Esta lógica de contorno garante que apenas os valores de elevação reais sejam considerados na normalização, preservando a fidelidade topográfica do terreno original.
...
\begin{itemize}
    ...
    \item \textbf{Avisos e recomendações}: Alertas sobre sistema de coordenadas, detecção de padding e outras informações relevantes.
\end{itemize}
