...
O QGIS é um Sistema de Informação Geográfica (SIG) livre, gratuito e multiplataforma, que suporta a visualização, edição e análise de dados geoespaciais. Sendo o software geoespacial livre mais popular do mundo, o QGIS é um projeto da Open Source Geospatial Foundation (OSGeo) e é mantido por uma crescente comunidade de usuários e desenvolvedores~\citep{khan2018,rosas2022}.
...
O \emph{GeoTIFF} é um padrão de domínio público que define um conjunto de metadados para descrever informações cartográficas e geodésicas associadas a imagens no formato TIFF (Tagged Image File Format)~\citep{ogcGeoTIFF}. Em essência, o GeoTIFF não é um novo formato de imagem, mas sim uma extensão do padrão TIFF 6.0, que utiliza o mecanismo de ``tags'' (etiquetas) para incorporar informações de georreferenciamento diretamente no arquivo de imagem. Esta abordagem cria um arquivo autocontido, onde os dados da imagem (os pixels) e os metadados que descrevem sua localização espacial coexistem.
