...
A escolha pela arquitetura de plugins do QGIS demonstrou-se acertada em múltiplos aspectos. A estrutura modular e extensível do QGIS, que privilegia a criação de funcionalidades especializadas através de plugins~\citep{qgisPlugins}, facilitou significativamente o processo de desenvolvimento e contribuição. A documentação abrangente da API PyQGIS, as convenções estabelecidas pela comunidade e a integração com o Processing Framework permitiram que a solução fosse desenvolvida de forma estruturada e alinhada com as práticas do ecossistema.
